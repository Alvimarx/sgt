...
my() { $COMPOSE exec -T mysql mysql -uroot -p"$MYSQL_ROOT_PASSWORD" --default-character-set=utf8mb4 "$@" 2>&1 | grep -v "Using a password on the command line" || true; }
...
if [ "${COL:-0}" -eq 0 ]; then
  ...
  my < BaseDatosMySQL/migraciones/V2__planificacion_vigencia.sql
else
